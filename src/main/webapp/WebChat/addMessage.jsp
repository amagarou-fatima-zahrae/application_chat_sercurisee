...
<p id="id_rec" class="d-none">${id_rec}</p>
	...
	const urlParams = new URLSearchParams(queryString);
	 ...
	 var socket = new WebSocket("ws://100.104.160.85:8081/WebChat/ServerEndPoint?userID="+id+"="+urlParams.get('id'));
	 socket.onopen = function(e) {
		...
		socket.onmessage=function(event){
			  ...
			  if (objectR.type === 'text') { 
				    ...
				    messageElement.innerHTML = '<div class="msg_cotainer_send"><i class="fas fa-download text-primary mx-2 px-3"   onclick="downloadText('+cpt+')"></i>'+objectR.content+'</div><div class="img_cont_msg"><img src="${user_emet.photo}" class="rounded-circle user_img_msg"></div>';
				...
				//	messageElement.innerHTML = '<video class="w-50 mb-3 h-50" controls><source src="' +contenu+ '"></video><a href="'+contenu+'" download><i class="fas fa-download"></i></a>';
	            	  ...
	            	  console.log("audio "+objectR.content);
	            	  var contenu =objectR.content.substring(0,objectR.content.length-2);
	            	  console.log('apres longeur msg '+contenu+" "+contenu.length);
		...
		function deconnecter(){
			socket.close();
			 window.location.href = 'http://100.104.160.85:8081/WebChat/Login';
		}
    ...
    //******************** END WEB SOCKETS*************************************************
	   /*  send Message */
		function sendMessage(type,url,file) {
	     ...
	     xhr.open('POST', 'http://100.104.160.85:8081/WebChat/AjaxCall');
	     xhr.onload = function() {
	       if (xhr.status === 200) {
...
window.addEventListener("beforeunload", function(event) {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    id=urlParams.get('id');
    const url = 'http://100.104.160.85:8081/WebChat/AjaxCall2?id='+id+'&id2='+id2;
    console.log("i'm quit");
